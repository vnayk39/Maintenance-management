...
const selectAllPayrecCheckbox = document.getElementById('selectAllPayrecCheckbox');
...
async function fetchPayments() {
    showLoading(listLoadingSpinner);
    try {
        ...
        renderPaymentList();
        hideLoading(listLoadingSpinner);
    } catch (error) {
...
function renderPaymentList() {
    ...
    if (filteredPayments.length === 0) {
        ...
        document.getElementById("selectboxcontainer").innerHTML = "";
        return;
    }
    ...
    filteredPayments.forEach(payment => {
        ...
        paymentItem.innerHTML = `
            <div class="payment-item-select">
                <input type="checkbox"
                    class="row-select-checkbox bulk-select"
                    data-payment-id="${payment.id}"
                    onclick="event.stopPropagation();
                    this.closest('.payment-item').classList.toggle('active', this.checked);
                    togglePrintAllBtn();" />
            </div>
            <div>
        ...
        //const delArg = deletePayment(payment.id);
        initBulkCheckboxHandlers(
            "paymentList",                      // tableId
            "selectAllPayrecCheckbox",         // select-all checkbox
            "payrec-bulkDeleteBtn",            // bulk delete button
            deletePayment,                      //row delete function (id, rowElement)
            delArg = true
        );
    });
}
...
async function multiMonthsPayrec(){
    ...
    const readonlyFields = ['periodic_building_maintenance', 'repair_and_maintenance_fund','sinking_funds']
    const mnths = document.querySelector('[name="months"]').value;

    fieldstoMulti.forEach(fieldName => {
        const field = document.querySelector(`[name="${fieldName}"]`);

        if (!field) return;
        //const baseValue = parseFloat(fetchedCharges[fieldName]) || 0;
        if (mnths === 1) {
            field.value = parseFloat(fetchedCharges[fieldName]).toFixed(2);
            if (readonlyFields.includes(fieldName)){rewriteInput(fieldName, field.value, readonly = true);}
            else{rewriteInput(fieldName, field.value);}

        } else {
            field.value = parseFloat(fetchedCharges[fieldName] * mnths).toFixed(2);
            if (readonlyFields.includes(fieldName)){rewriteInput(fieldName, field.value, readonly = true);}
            else{rewriteInput(fieldName, field.value);}
        }
    });
    ...
    if (fetchedCharges.additional_charges) {
        Object.entries(fetchedCharges.additional_charges).forEach(([key, val]) => {
            const input = document.querySelector(`[name="additional_charges_${key}"]`);
            if (!input) return;

            const baseValue = parseFloat(val) || 0;
            if (mnths === 1) {
                input.value = baseValue.toFixed(2);
            } else {
                input.value = (baseValue * mnths).toFixed(2);
            }
        });
    }
    calculateEditTotal();
...
function rewriteInput(name, value, readonly = false) {
        ...
        detailContent.innerHTML = `
            <form id="paymentEditForm">
                ...
                <div class="detail-row">
                    ...
                    <span class="detail-value">
                        <input type="number" class="form-control form-control-sm" name="months" value="${payment.months}"  oninput="multiMonthsPayrec()" min="1">
                    </span>
                </div>
            <div class="detail-section">
                ...
                <div class="detail-row">
                    ...
                    <span id= "periodic_building_maintenance-container" class="detail-value">
                        <input type="number" class="form-control form-control-sm charge-input"
                            name="periodic_building_maintenance" value="${payment.periodic_building_maintenance}" readonly style="background-color: #f0f0f0;">
                    </span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Repair & Maintenance Fund:</span>
                    <span id= "repair_and_maintenance_fund-container" class="detail-value">
                        <input type="number" class="form-control form-control-sm charge-input"
                            name="repair_and_maintenance_fund" value="${payment.repair_and_maintenance_fund}" readonly style="background-color: #f0f0f0;">
                    </span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Sinking Funds:</span>
                    <span id= "sinking_funds-container" class="detail-value">
                        <input type="number" class="form-control form-control-sm charge-input"
                            name="sinking_funds" value="${payment.sinking_funds}" readonly style="background-color: #f0f0f0;">
                    </span>
                </div>
                ${payment.additional_charges ? Object.entries(payment.additional_charges)
                .map(([key, value]) => `
                    <div class="detail-row">
                        <span class="detail-label">${key}:</span>
                        <span class="detail-value">
                        <input type="number" class="form-control form-control-sm charge-input" name="additional_charges_${key}" value="${parseFloat(value).toFixed(2)}" readonly style="background-color: #f0f0f0;">
                        </span>
                    </div>
                `).join('') : ''}
                <div class="detail-row">
                    <span class="detail-label">Parking:</span>
                    <span id= "parking-container" class="detail-value">
                        <input type="number" step="0.01" class="form-control form-control-sm charge-input"
                            name="parking" value="${payment.parking}">
                    </span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Penalty:</span>
                    <span id= "penalty-container" class="detail-value">
                        <input type="number" step="0.01" class="form-control form-control-sm charge-input"
                ...
                <div class="detail-row">
                    ...
                    <span id= "lease_rent-container" class="detail-value">
                        <input type="number" step="0.01" class="form-control form-control-sm charge-input"
                            name="lease_rent" value="${payment.lease_rent}">
                    </span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Balance:</span>
                    <span id= "balance-container" class="detail-value">
                        <input type="number" step="0.01" class="form-control form-control-sm charge-input"
...
function calculateEditTotal() {
    ...
    document.querySelectorAll('[name^="additional_charges_"]').forEach(input => {
        total += parseFloat(input.value) || 0;
    });
    ...
    if (totalInput) {
        ...
        requestAnimationFrame(() => totalInput.value = total.toFixed(2));
    }
}
...
async function savePaymentChanges() {
    ...
    form.querySelectorAll('[name^="additional_charges_"]').forEach(input => {
        const key = input.name.replace('additional_charges_', '');
        additionalCharges[key] = parseFloat(input.value) || 0;
    });
    ...
    const updatedPayment = {
        ...
        periodic_building_maintenance: parseFloat(formData.get('periodic_building_maintenance')),
        repair_and_maintenance_fund: parseFloat(formData.get('repair_and_maintenance_fund')),
        sinking_funds: parseFloat(formData.get('sinking_funds')),
        additional_charges: additionalCharges,
        parking: parseFloat(formData.get('parking')),
        penalty: parseFloat(formData.get('penalty')),
        lease_rent: parseFloat(formData.get('lease_rent')),
        balance: parseFloat(formData.get('balance')),
        //total_maintenance_amount: parseFloat(formData.get('total_maintenance_amount')),
        get total_maintenance_amount() {
            const additionalChargesTotal = Object.values(this.additional_charges || {}).reduce((acc, val) => acc + val, 0);
...
async function deletePayment(paymentId, bulk=null) {
    // Simple confirmation
    if(!bulk){
...
function viewReceipt(payment) {
    ...
    window.open(`/static/myapp/rec_tempp.html?data=${encoded}`, '_blank');
}

function printAll(prnt=false) {
  const selectedBoxes = document.querySelectorAll('.row-select-checkbox:checked');
  if (selectedBoxes.length === 0) {
    showNotification('No payments selected for printing.', 'warning');
    return;
  }

  // Fetch payment objects for each selected id
  const selectedPayments = [];
  selectedBoxes.forEach(cb => {
    const id = parseInt(cb.dataset.paymentId);
    const record = allPayments.find(p => p.id === id);
    if (record) selectedPayments.push(record);
  });

  if (selectedPayments.length > 0) {
    const encoded = encodeURIComponent(JSON.stringify(selectedPayments));
    if(prnt){const newWindow = window.open(`/static/myapp/rec_tempp.html?data=${encoded}&autoPrint=true`, '_blank');}
    else{window.open(`/static/myapp/rec_tempp.html?data=${encoded}&viewAll=true`, '_blank');}
  }
}

function togglePrintAllBtn() {
  const printAllBtn = document.getElementById("payrec-printAll");
  const viewAllBtn = document.getElementById("payrec-viewAll");
  if (!printAllBtn || !viewAllBtn) return;

  const anyChecked = document.querySelectorAll('.row-select-checkbox:checked').length > 0;
  printAllBtn.style.display = anyChecked ? "block" : "none";
  viewAllBtn.style.display = anyChecked ? "block" : "none";
}
...
document.getElementById('payrec-printAll')?.addEventListener('click', ()=>{printAll(true)});
document.getElementById('payrec-viewAll')?.addEventListener('click', ()=>{printAll(false)});
selectAllPayrecCheckbox.addEventListener('change', (e) => {
