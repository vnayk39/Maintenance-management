...
const responseMessageCharges = document.getElementById('responseMessageCharges');
const loadingSpinnerCharges = document.getElementById('loadingSpinnerCharges');

async function fetchSpacewiseCharges() {
    showLoading(loadingSpinnerCharges, responseMessageCharges, "Fetching spacewise charges...");
    try {
        const response = await fetch(`${API_BASE_URL}charges/`);
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const data = await response.json();
        renderChargesTable(data);
        hideLoading(loadingSpinnerCharges, responseMessageCharges, "Spacewise charges loaded.");
    } catch (error) {
        hideLoading(loadingSpinnerCharges, responseMessageCharges, `Error loading spacewise charges: ${error.message}`, true);
...
function setCharge() {
    ...
    spacewiseChargesTableBody.querySelectorAll("tr").forEach(row => {
        ...
        chrgeVal.value = 0;
        setTimeout(() => {
            hideLoading(loadingSpinnerCharges, responseMessageCharges, `Added new charge: '${chrgeName}' successfully!`);
            renderChargesHeader();
...
function addChargesTableRow(rowData = {}) {
    ...
    fields.forEach(field => {
        ...
        if (field.type === 'select') {
            ...
            cell.style.display = "none";
            createExtraChargeCell(newRow, "", 0, set=true);
            return;
        }
        else {
    ...
    removeBtn.addEventListener('click', async () => {
        ...
        if (rowId && confirm(`Are you sure you want to delete charges for ${rowData.space_type}?`)) {
            ...
            newRow.remove();
            hideLoading(loadingSpinnerCharges, responseMessageCharges, "Unsaved row removed.", false);
        }
    });
...
function createExtraChargeCell(row, key, value, set=false) {
    ...
    if(!set){
        input.name = key;
        input.value = value || parseFloat(0).toFixed(2);
        ...
        input.id = `chrge_${spacewiseChargesTableBody.rows.length}`;
...
async function saveChargesRowData(row, changedFieldName, changedValue, rem=false) {
    ...
    row.querySelectorAll('input, select, textarea').forEach(element => {
        ...
        if (element.name === 'additional_charges' || element.name === 'charge_standin') return;
    ...
    showLoading(loadingSpinnerCharges, responseMessageCharges, "Saving data...");
    ...
    try {
        ...
        hideLoading(loadingSpinnerCharges, responseMessageCharges, "Data saved successfully!");
        if (!rowId) {
            // addChargesTableRow({});
        }
    } catch (error) {
        hideLoading(loadingSpinnerCharges, responseMessageCharges, `Error saving: ${error.message}`, true);
...
async function deleteChargesRowData(rowId, rowElement) {
    showLoading(loadingSpinnerCharges, responseMessageCharges, "Deleting data...");
    try {
        ...
        if (response.status === 204) {
            rowElement.remove();
            hideLoading(loadingSpinnerCharges, responseMessageCharges, "Charges deleted successfully!");
        ...
        hideLoading(loadingSpinnerCharges, responseMessageCharges, `Error deleting: ${error.message}`, true);
