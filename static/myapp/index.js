// index
    ...
    const responseMessageMaint = document.getElementById('responseMessageMaint');
    const loadingSpinnerMaint = document.getElementById('loadingSpinnerMaint');
    ...
    async function fetchInitialMaintenanceData() {
        showLoading(loadingSpinnerMaint, responseMessageMaint, "Loading data...");
        try {
            ...
            hideLoading(loadingSpinnerMaint, responseMessageMaint, "Data loaded successfully!");
        } catch (error) {
            hideLoading(loadingSpinnerMaint, responseMessageMaint, `Error loading data: ${error.message}`, true);
...
//  end
    ...
    function addMaintenanceTableRow(rowData = {}) {
        ...
        removeBtn.addEventListener('click', async () => {
            ...
            if (rowId) {
                ...
                newRow.remove();
                hideLoading(loadingSpinnerMaint, responseMessageMaint, "Unsaved row removed.", false);
            }
        });
    ...
    function createMaintExtraChargeCell(row, key, value) {
        ...
        input.type = 'number';
        input.name = key;
        input.value = parseFloat(0).toFixed(2);
        input.readOnly = true;
    ...
    function autoFillMaintenanceCharges(row, spaceTypeId, applyLeaseRent, num_of_vehicles) {
        ...
        // NEW: Auto-fill additional charges
           ...
           if (row.dataset.id && !row.dataset.justSaved) {
            row.dataset.justSaved = "true";
            saveMaintenanceRowData(row, "autoFill", null).finally(() => {
                delete row.dataset.justSaved;
            });
            }
    ...
    function updateMaintenanceRowTotal(row) {
        ...
        const total = calculateMaintenanceTotal(row, rowData);
    ...
    async function saveMaintenanceRowData(row, changedFieldName, changedValue) {
        ...
        row.querySelectorAll('input, select').forEach(element => {
            if (element.classList.contains('extra-charge-input')) return;
            if (element.name === 'space_type') {
                ...
                rowData[element.name] = row.querySelector('[name="total"]').value
            }
            else {
        ...
        showLoading(loadingSpinnerMaint, responseMessageMaint, "Saving data...");
        ...
        try {
            ...
            row.querySelector('[name="total"]').value = calculateMaintenanceTotal(row, rowData);
            // if (responseData.additional_charges) {
            //     // update row.dataset so later merges start fresh
            //     row.dataset.additional_charges = JSON.stringify(responseData.additional_charges);

            //     Object.entries(responseData.additional_charges).forEach(([key, val]) => {
            //         const input = row.querySelector(`[name="${key}"]`);
            //         if (input) {
            //             input.value = parseFloat(val || 0).toFixed(2);
            //         } else {
            //             createMaintExtraChargeCell();
            //         }
            //     });
            // }
            hideLoading(loadingSpinnerMaint, responseMessageMaint, "Data saved successfully!");
        } catch (error) {
            hideLoading(loadingSpinnerMaint, responseMessageMaint, `Failed to save data: ${error.message}`, true);
    ...
    async function deleteMaintenanceRowData(rowId, rowElement) {
        showLoading(loadingSpinnerMaint, responseMessageMaint, "Deleting data...");
        try {
            ...
            rowElement.remove();
            hideLoading(loadingSpinnerMaint, responseMessageMaint, "Entry deleted successfully!");
        } catch (error) {
            hideLoading(loadingSpinnerMaint, responseMessageMaint, `Failed to delete data: ${error.message}`, true);
