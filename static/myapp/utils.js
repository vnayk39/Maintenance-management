const API_BASE_URL = 'https://vnayk39.pythonanywhere.com/api/';
let EXTRA_CHARGE_KEYS = new Set();

// function showLoading(loadingSpinner, responseMessage, message = "Processing...") {
//     loadingSpinner.style.display = 'inline-block';
//     responseMessage.textContent = message;
//     responseMessage.style.color = 'orange';
//     }

// function hideLoading(loadingSpinner, responseMessage, message = "", isError = false) {
...
function initBulkCheckboxHandlers(tableId, selectAllCheckboxId, bulkDeleteBtnId, deleteHandler, delArg =  null) {
    ...
    if (!table || !selectAllCheckbox || !bulkDeleteBtn) {
        console.warn("Bulk checkbox utilities: table or controls not found!");
        return;
    }

    // Toggle all row checkboxes on "select all"
    selectAllCheckbox.addEventListener('change', (e) => {
        const checked = e.target.checked;
        table.querySelectorAll(`#${tableId} .row-select-checkbox`).forEach(cb => {
            cb.checked = checked;
        });
        toggleBulkDeleteButton(tableId, bulkDeleteBtnId);
    });

    // Watch for individual row changes
    ...
    // Bulk delete action
    if (!bulkDeleteBtn.dataset.bound) {
        bulkDeleteBtn.addEventListener('click', async () => {
            const selected = table.querySelectorAll('.row-select-checkbox:checked');
            ...
            if (!confirm(`Are you sure you want to delete ${selected.length} item(s)?`)) return;
             ...
             for (const cb of selected) {
                ...
                await deleteHandler(paymentId, bulk = true);
                }
                else{
                    const row = cb.closest('tr');
                    const rowId = row.dataset.id;
                    if (rowId) {await deleteHandler(rowId, row);}
                    else {row.remove();}
                }
            }
            toggleBulkDeleteButton(tableId, bulkDeleteBtnId);
...
function toggleBulkDeleteButton(tableId, bulkDeleteBtnId) {
    const table = document.getElementById(tableId);
    const bulkDeleteBtn = document.getElementById(bulkDeleteBtnId);

    if (!table || !bulkDeleteBtn) return;

    const anySelected = table.querySelectorAll('.row-select-checkbox:checked').length > 0;
    bulkDeleteBtn.style.display = anySelected ? 'block' : 'none';
}
...
function initPaymentsPage() {
    ...
    // document.getElementById("paymentstab").addEventListener('click', addPaymentRow);
    }
}
